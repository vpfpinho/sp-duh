    ...
    execute %Q[
      ...
      BEGIN
        ...
        IF left(a_schema_name,4) = 'user' THEN
          ...
          _tablespace_name := lpad( split_part(a_schema_name,'_',2), 3, '0');
        ELSIF left(a_schema_name,11) = 'pt999999990' THEN
          ...
          _tablespace_name := right( regexp_replace(a_schema_name,'^pt\d{6}(\d{3}).*?(\d*)$','\1\2'), 3);
        ELSE
          -- last 3 digits from tax_registration_number
          _tablespace_name := substr(a_schema_name,9,3);
        END IF;
