class AddedFunctionsToSimplifyEnablingAndDisablingOfTriggersOnAnyTable < ActiveRecord::Migration
  def up
    execute <<-'SQL'
  ...
  def down
    execute %Q[DROP FUNCTION IF EXISTS common.enable_triggers_on_table(TEXT, VARIADIC TEXT[]);]
    execute %Q[DROP FUNCTION IF EXISTS common.disable_triggers_on_table(TEXT, VARIADIC TEXT[]);]
