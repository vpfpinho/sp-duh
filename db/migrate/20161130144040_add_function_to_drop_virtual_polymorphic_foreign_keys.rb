class AddFunctionToDropVirtualPolymorphicForeignKeys < ActiveRecord::Migration
  def up
    execute <<-'SQL'
  ...
  def down
    execute %Q[DROP FUNCTION IF EXISTS sharding.drop_virtual_polymorphic_foreign_key(TEXT, TEXT, JSONB, TEXT);]
    execute %Q[DROP FUNCTION IF EXISTS sharding.get_drop_virtual_polymorphic_foreign_key_queries(TEXT, TEXT, JSONB, TEXT);]
  end
end
