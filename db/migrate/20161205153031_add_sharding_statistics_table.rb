class AddShardingStatisticsTable < ActiveRecord::Migration
  def up
    execute <<-'SQL'
      CREATE TABLE sharding.sharding_statistics (
        sharding_key                  INTEGER PRIMARY KEY,
        structure_sharding_started_at TIMESTAMP WITH TIME ZONE,
        structure_sharding_ended_at   TIMESTAMP WITH TIME ZONE,
        data_sharding_started_at      TIMESTAMP WITH TIME ZONE,
        data_sharding_ended_at        TIMESTAMP WITH TIME ZONE
      );
    SQL
  ...
  def down
    ...
    execute %Q[DROP TABLE sharding.sharding_statistics;]
  end
end
