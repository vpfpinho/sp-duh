class AddProgressToShardingStatistics < ActiveRecord::Migration
  def up
    execute <<-'SQL'
      CREATE TYPE sharding.sharding_status AS ENUM(
        ...
        , 'error-copying-data'
        , 'error-deleting-original-data'
        , 'success'
      );
    SQL

    execute <<-'SQL'
      CREATE TYPE sharding.sharding_triggered_by AS ENUM(
          'insert'
        , 'update'
      );
    SQL
    ...
    execute <<-'SQL'
      ALTER TABLE sharding.sharding_statistics
          ADD COLUMN status sharding.sharding_status NOT NULL DEFAULT 'not-started'
        , ADD COLUMN current_step TEXT
        , ADD COLUMN current_step_at TIMESTAMP WITH TIME ZONE
        , ADD COLUMN triggered_by sharding.sharding_triggered_by NOT NULL DEFAULT 'insert'
      ;
    SQL
    ...
    execute <<-'SQL'
      CREATE TRIGGER trg_updated_sharding_statistics_step
      BEFORE INSERT OR UPDATE OF status, current_step ON sharding.sharding_statistics
      FOR EACH ROW EXECUTE PROCEDURE sharding.trf_updated_sharding_statistics_step();
    SQL
    ...
    # Data update (no rollback)
    execute <<-'SQL'
      UPDATE sharding.sharding_statistics
      SET triggered_by = (CASE WHEN data_sharding_started_at IS NOT NULL THEN 'update' ELSE 'insert' END)::sharding.sharding_triggered_by;
    SQL
  end
  ...
  def down
    ...
    execute <<-'SQL'
      DROP TRIGGER IF EXISTS trg_updated_sharding_statistics_step ON sharding.sharding_statistics;
    SQL

    execute <<-'SQL'
      DROP FUNCTION IF EXISTS sharding.trf_updated_sharding_statistics_step();
    SQL
    ...
    execute <<-'SQL'
      ALTER TABLE sharding.sharding_statistics
        ...
        , DROP COLUMN current_step_at
        , DROP COLUMN triggered_by
      ;
    SQL

    execute %Q[DROP TYPE IF EXISTS sharding.sharding_triggered_by;]
    execute %Q[DROP TYPE IF EXISTS sharding.sharding_status;]
  end
end
